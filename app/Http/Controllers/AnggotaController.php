...
class AnggotaController extends Controller
{
    ...
    public function show(string $id, \Illuminate\Http\Request $request)
    {
        ...
        $transaksiQuery = $anggota->transaksis()->with('buku')->latest();

        $filterStatus = $request->query('status');
        if ($filterStatus && in_array($filterStatus, ['Dipinjam', 'Dikembalikan'])) {
            $transaksiQuery->where('status', $filterStatus);
        }

        $transaksis = $transaksiQuery->get();

        // Statistik peminjaman
        $allTransaksis = $anggota->transaksis;
        $statsPeminjaman = [
            'total_pinjam'   => $allTransaksis->count(),
            'total_denda'    => $allTransaksis->sum('denda'),
            'masih_dipinjam' => $allTransaksis->where('status', 'Dipinjam')->count(),
        ];

        return view('anggota.show', compact('anggota', 'transaksis', 'statsPeminjaman', 'filterStatus'));
    }
